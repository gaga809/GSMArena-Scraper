...
export async function fetchPage(url: string): Promise<cheerio.CheerioAPI> {
  return new Promise<cheerio.CheerioAPI>((resolve, reject) => {
    if (!CAN_FETCH) {
      return reject(
        new Error("Currently rate limited. Please wait before retrying.")
      );
    }
...
export function extrapolateTextNoHTML(
...
export function extrapolateTextNoHTMLFromElement(
...
export function extrapolateAttr(
  document: cheerio.CheerioAPI,
  query: string,
  attr: string
) {
...
export function extrapolateAttrFromElement(
  element: cheerio.Cheerio<AnyNode>,
  attr: string
) {
...
export function getChildOfParent(
  document: cheerio.CheerioAPI,
  parentQuery: string,
  childQuery: string
) {
...
export function loadCamera(
  ...
  const cameras = extrapolateText(
    document,
    `td[data-spec="cam${camN}modules"]`
  );
  ...
  const cameraFeatures = extrapolateText(
    document,
    `td[data-spec="cam${camN}features"]`
  );
  const cameraFeaturesList = cameraFeatures
    ? cameraFeatures.split(",").map((feature) => feature.trim())
    : [];

  const cameraVideo = extrapolateText(
    document,
    `td[data-spec="cam${camN}video"]`
  );
  const cameraVideoList = cameraVideo
    ? cameraVideo
        .split(";")[0]
        .trim()
        .split(",")
  ...
  const cameraOtherList = cameraVideo.split(";")[1]
    ? cameraVideo
        .split(";")[1]
        .trim()
        .split(",")
...
export function extrapolateDate(gsmarenaDate: string) {
  try {
    ...
    return `${dateYear}-${dateMonth.toString().padStart(2, "0")}-${dateDay
      .toString()
      .padStart(2, "0")}`;
  } catch {
...
function fromStringToMonthNumber(monthString: string): number {
  ...
  return monthMap[monthString.toLowerCase()] !== undefined
    ? monthMap[monthString.toLowerCase()]
    : -1;
}
